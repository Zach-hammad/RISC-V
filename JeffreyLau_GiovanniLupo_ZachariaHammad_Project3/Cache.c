...
const unsigned cache_size = 126; // Size of a cache (in KB)
...
bool lfu(Cache *cache, uint64_t addr, Cache_Block **victim_blk, uint64_t *wb_addr)
{
    ...
    int min_frq = ways[0]->frequency;
    for (i = 0; i < cache->num_ways; i++)
    {
        if (ways[i]->frequency < min_frq)
        {
            min_frq = ways[i]->frequency;
        }
    }

    // Step two, if there is no invalid block. Locate the LRU block
    Cache_Block *victim = ways[0];
    uint64_t oldest = UINTMAX_MAX;
    for (i = 1; i < cache->num_ways; i++)
    {
        if (ways[i]->frequency == min_frq)
        {
            if(ways[i]->when_touched < oldest)
            {
                oldest = ways[i]->when_touched;
                victim = ways[i];
            }
        }
    }
